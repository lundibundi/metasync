...
module.exports = (api) => {
  const collectorMethodNames = ['pick', 'fail', 'take', 'timeout', 'done'];
  ...
  api.metasync.collect = (expected) => {
    ...
    let isDone = false;
    const data = {};
    const returnIfDone = (fn) => (...args) => {
      if (!isDone) return fn(...args);
    };

    const collector = returnIfDone((key, err, value) => {
    ...
    collector.pick = (key, value) => collector(key, null, value);

    collector.fail = (key, err) => collector(key, err);

    collector.take = (key, fn, ...args) => {
      if (count + takeCbCount < expected) {
        takeCbCount++;
        fn(...args, (...cbArgs) => {
          takeCbCount--;
          collector(key, ...cbArgs);
        });
      }
      return collector;
    }
    ...
    collector.done = (cb) => {
        ...
        isDone = true;        
        for (let methodName of collectorMethodNames) {
          collector[methodName] = api.common.emptiness;
        }
        cb(...args);
      };
      return collector;
    ...
    for (let methodName of collectorMethodNames) {
      collector[methodName] = returnIfDone(collector[methodName]);
    }
